...
cl::opt<bool> TauDryRun("tau-dry-run",
                        cl::desc("Don't actually instrument the code, "
                                 "just print what would be instrumented"));

cl::opt<std::string> TauSelectFile(
    "tau-select",
    cl::desc(
        "File to write selection info to (truncated/created as necessary)"),
...
struct Instrument : public FunctionPass {
   ...
   */
  void maybeSaveForProfiling(CallInst *call,
                             SmallVectorImpl<CallAndName> &calls) {
...
// Instrument outermost parallel-annotated loops
struct ProfLoop : public LoopPass {

  static char ID; // Pass identification, replacement for typeid

  std::ofstream ofile;

  ProfLoop() : LoopPass(ID) {
    if (!TauInputFile.empty()) {
      ofile = std::ofstream(TauSelectFile);
    }
  }
  ...
  bool shouldInstrument(Loop *L) {
    return noParallelParents(L) && L->isAnnotatedParallel();
  }

  bool noParallelParents(const Loop *L) {
    // TODO: Check depth instead?
    const Loop *parent = L->getParentLoop();
    return !parent || (!parent->isAnnotatedParallel() &&
                       noParallelParents(parent));
  }
...
static RegisterPass<Instrument> I("tau-prof", "TAU Profiling", false, false);
...
// Not sure if this needs to be distinct from other <Pass>::ID values
char ProfLoop::ID = 1;

static RegisterPass<ProfLoop> PL("tau-loop", "TAU Loop Profiling", false, false);

// Automatically enable the pass.
// http://adriansampson.net/blog/clangpass.html
static void registerProfLoop(const PassManagerBuilder &,
                             legacy::PassManagerBase &PM) {
  PM.add(new ProfLoop());
}

static RegisterStandardPasses
    RegisterProfLoop(PassManagerBuilder::EP_EnabledOnOptLevel0,
                     registerProfLoop);
